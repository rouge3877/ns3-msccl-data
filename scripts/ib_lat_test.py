...
print(f"Starting 16x16 IB Latency Test (Complete matrix test)...")
print(f"Total tests to run: {len(DEVICES) * 2 * len(DEVICES) * 2}\n")

# 测试函数
def test_latency(client_dev, server_dev, server_ip, client_is_remote, server_is_remote, port_offset):
    ...
    print(f"Testing: {client_loc}[{client_dev}] -> {server_loc}[{server_dev}] (Port: {current_port})")
    ...
    if server_is_remote:
        ...
        # 本地服务端
        server_cmd = f"ib_write_lat -d {server_dev} -p {current_port}"
        srv_proc = subprocess.Popen(server_cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    if client_is_remote:
        ...
        # 客户端需要连接到服务端的IP
        client_cmd = f"ssh {SERVER_SSH_HOST} 'ib_write_lat -d {client_dev} -p {current_port} {server_ip} --perform_warm_up -F'"
...
for client_idx, client_dev in enumerate(DEVICES):
    ...
    for server_idx, server_dev in enumerate(DEVICES):
        ...
        latency_val = test_latency(client_dev, server_dev, LOCAL_IP, 
                                   client_is_remote=False, server_is_remote=False, 
                                   port_offset=port_offset)
        row_data[f"Local_{server_dev}"] = latency_val
    
    # 再测试远程8张卡作为服务端
    for server_idx, server_dev in enumerate(DEVICES):
        port_offset = (client_idx * len(DEVICES) * 2) + len(DEVICES) + server_idx
        latency_val = test_latency(client_dev, server_dev, SERVER_IP, 
                                   client_is_remote=False, server_is_remote=True, 
                                   port_offset=port_offset)
        row_data[f"Remote_{server_dev}"] = latency_val
...
for client_idx, client_dev in enumerate(DEVICES):
    ...
    for server_idx, server_dev in enumerate(DEVICES):
        ...
        latency_val = test_latency(client_dev, server_dev, LOCAL_IP_FOR_REMOTE, 
...
# 保存到 CSV，方便你在仿真软件中导入
csv_filename = "ib_latency_matrix_16x16.csv"
